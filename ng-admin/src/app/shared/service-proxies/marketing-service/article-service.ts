...
export class ArticleServiceProxy {
    ...
    get(id: number): Observable<Article> {
        let url_ = this.baseUrl + "/api/services/app/Article/GetArticleByIdAsync?";
        if (id !== undefined)
        ...
        return this.http.request(url_, options_).flatMap((response_) => {
            ...
            if (response_ instanceof Response) {
                try {
                    ...
                    return <Observable<Article>><any>Observable.throw(e);
                }
            } else
                return <Observable<Article>><any>Observable.throw(response_);
        });
    }

    protected processGet(response: Response): Observable<Article> {
        const status = response.status;
        ...
        if (status === 200) {
            ...
            let result200: any = null;
            let resultData200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver);
            result200 = resultData200 ? Article.fromJS(resultData200) : new Article();
            return Observable.of(result200);
        } else if (status === 401) {
            const _responseText = response.text();
            return throwException("A server error occurred.", status, _responseText, _headers);
        ...
        return Observable.of<Article>(<any>null);
    }

    update(input: Article): Observable<Article> {
        let url_ = this.baseUrl + "/api/services/app/Article/CreateOrUpdateArticleDto";
        url_ = url_.replace(/[?&]$/, "");
    ...
    protected processUpdate(response: Response): Observable<Article> {
        const status = response.status;
    ...
    delete(id: string): Observable<void> {
        let url_ = this.baseUrl + "/api/services/app/Article/DeleteArticle?";
        if (id !== undefined)
            url_ += "Id=" + encodeURIComponent("" + id) + "&";
        url_ = url_.replace(/[?&]$/, "");
    ...
    ExportExcel(input:any):Observable<ApiResult> {
        let url_ = this.baseUrl + "/api/services/app/Article/ExportArticlesExcel";
        url_ = url_.replace(/[?&]$/, "");
